...
if(isset($_POST['settings_submit'])){
	if(!empty($_POST['settings_submit'])){
		...
		if(isset($_POST['deleteallrecords']) and $delete_setting=='yes'){
			$setting_result = update_option('woocity_delete_data',$delete_setting);
		}
		else{
			$setting_result = update_option('woocity_delete_data','no');
		}
...
?>
<div class="woocity-settings">
<div class="container">
	<h2>Woo City Massindo Settings</h2>
	<form action="" method="post" name="woocity-settings-form">
		...
		<div class="form-group settings-form-group">
			...
			<input type="checkbox" name="deleteallrecords" value='yes' class="form-control" <?php if(get_option( 'woocity_delete_data') == 'yes'): ?> checked <?php endif?>>
		</div>
